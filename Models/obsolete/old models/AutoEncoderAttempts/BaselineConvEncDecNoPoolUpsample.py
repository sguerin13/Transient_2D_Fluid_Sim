...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.dconv22  = nn.Conv2d(256,128,3,stride = 1,padding=1)
        ...
        self.dconv32  = nn.Conv2d(128,64,3,stride = 1,padding=1)
        ...
        self.dconv44  = nn.Conv2d(64,64,3,stride = 1,padding=1)
        torch.nn.init.xavier_uniform(self.dconv44.weight)
        ...
        self.dconv53  = nn.Conv2d(64,64,3,stride = 1,padding=1)
        torch.nn.init.xavier_uniform(self.dconv53.weight)
        self.bn53    = nn.BatchNorm2d(64) # probably not needed
        self.dconv54  = nn.Conv2d(64,3,3,stride = 1,padding=1)
        torch.nn.init.xavier_uniform(self.dconv54.weight)
        # self.bn44    = nn.BatchNorm2d(64)
    ...
    def forward(self,x):#,x,max_ind_1,max_ind_2):
        ...
        x = F.relu(self.bn44(self.dconv44(x)))
        
        x = self.up5(x)
        x = F.relu(self.bn52(self.dconv52(x)))
        x = F.relu(self.bn53(self.dconv53(x)))
        y = self.dconv54(x)
...
class EncoderDecoder(nn.Module):
    ...
    def train_net(self,dataset,lr = 1e-3,epochs = 100):
        ...
        self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, 'min',factor=.2,threshold = .001,patience =5)
        best_loss = 10
        self.loss_vec = []
